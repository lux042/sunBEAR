...
const standalone=element('Accessible standalone story paragraph');
const hiddenStandalone=element('Hidden standalone paragraph','P',{display:'none'});
const standaloneDoc={querySelectorAll:s=>s.startsWith('[data-testid="paragraph"]')?[standalone,hiddenStandalone]:[]};
const standaloneResult=runInNewContext(script,{document:standaloneDoc,location:{href:'https://www.nytimes.com/2026/01/01/world/test.html'},getComputedStyle:n=>n.style,Array,Set,Number});
assert.equal(standaloneResult.blocks.length,1);assert.equal(standaloneResult.blocks[0].text,standalone.innerText);
console.log('PASS Standalone NYT paragraph layout includes visible text only');
console.log('5 passed; 0 failed. Live NYT body extraction still requires verification.');
